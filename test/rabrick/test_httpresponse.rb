...
module Rabrick
  class TestHTTPResponse < Test::Unit::TestCase
    ...
    def test_prevent_response_splitting_cookie_headers_crlf
      ...
      res.cookies << Rabrick::Cookie.new('author', user_input)
      io = StringIO.new
      res.send_response io
      io.rewind
      res = Net::HTTPResponse.read_new(Net::BufferedIO.new(io))
    ...
    def test_set_redirect_response_splitting
      ...
      assert_raise(URI::InvalidURIError) do
        res.set_redirect(Rabrick::HTTPStatus::MultipleChoices, url)
      end
    end

    def test_set_redirect_html_injection
      url = 'http://example.com////?a</a><head></head><body><img src=1></body>'
      assert_raise(Rabrick::HTTPStatus::MultipleChoices) do
        res.set_redirect(Rabrick::HTTPStatus::MultipleChoices, url)
      end
      res.status = 300
      io = StringIO.new
    ...
    def test_set_error
      ...
      @res.status = status
      error = Rabrick::HTTPStatus[status].new(message)
