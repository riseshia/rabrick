...
module WEBrick
  ...
  module Utils
    ...
    def create_self_signed_cert(bits, cn, comment)
      rsa = if $VERBOSE
              OpenSSL::PKey::RSA.new(bits) { |p, _n|
                case p
                when 0 then $stderr.putc "."  # BN_generate_prime
                when 1 then $stderr.putc "+"  # BN_generate_prime
                when 2 then $stderr.putc "*"  # searching good prime,
                # n = #of try,
      ...
      cert.version = 2
      cert.serial = 1
      name = if cn.is_a? String
               OpenSSL::X509::Name.parse(cn)
             else
