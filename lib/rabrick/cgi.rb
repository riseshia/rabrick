...
module Rabrick
  # A CGI library using Rabrick requests and responses.
  #
  # Example:
  #
  #   class MyCGI < Rabrick::CGI
  #     def do_GET req, res
  #       res.body = 'it worked!'
  #       res.status = 200
  #     end
  ...
  class CGI
    ...
    ##
    # The CGI configuration.  This is based on Rabrick::Config::HTTP
    ...
    def initialize(*args)
      if defined?(MOD_RUBY) && !ENV.has_key?("GATEWAY_INTERFACE")
        Apache.request.setup_cgi_env
      end
      if %r{HTTP/(\d+\.\d+)} =~ ENV["SERVER_PROTOCOL"]
        httpv = ::Regexp.last_match(1)
      end
      @config = Rabrick::Config::HTTP.dup.update(
    ...
    def start(env = ENV, stdin = $stdin, stdout = $stdout)
      sock = Rabrick::CGI::Socket.new(@config, env, stdin, stdout)
    ...
    # Rabrick::HTTPServlet::AbstractServlet#service for details.
    ...
    class Socket # :nodoc:
      ...
      def request_line
        ...
        unless url = @env["REQUEST_URI"]
          ...
          url = Rabrick::HTTPUtils.escape_path(url)
      ...
      def setup_header
        @env.each { |key, value|
          ...
          when /^HTTP_(.*)/
            add_header(::Regexp.last_match(1).gsub(/_/, "-"), value)
          end
        }
      end
