...
module Rabrick
  module HTTPAuth
    ##
    # RFC 2617 Digest Access Authentication for Rabrick
    #
    # Use this class to add digest authentication to a Rabrick servlet.
    #
    # Here is an example of how to set up DigestAuth:
    #
    #   config = { :Realm => 'DigestAuth example realm' }
    #
    #   htdigest = Rabrick::HTTPAuth::Htdigest.new 'my_password_file'
    #   htdigest.set_passwd config[:Realm], 'username', 'password'
    #   htdigest.flush
    #
    #   config[:UserDB] = htdigest
    #
    #   digest_auth = Rabrick::HTTPAuth::DigestAuth.new config
    #
    # When using this as with a servlet be sure not to create a new DigestAuth
    # object in the servlet's #initialize.  By default Rabrick creates a new
    ...
    class DigestAuth
      ...
      #
      # See Rabrick::Config::DigestAuth for default configuration entries
      #
      # You must supply the following configuration entries:
      #
      # :Realm:: The name of the realm being protected.
      # :UserDB:: A database of usernames and passwords.
      #           A Rabrick::HTTPAuth::Htdigest instance should be used.
      ...
      def initialize(config, default = Config::DigestAuth)
        ...
        @instance_key = hexdigest(__id__, Time.now.to_i, Process.pid)
        @opaques = {}
      ...
      def _authenticate(req, res)
        ...
        req_params = if auth_req['qop'] == "auth" || auth_req['qop'] == "auth-int"
                       MustParams + MustParamsAuth
                     else
                       MustParams
                     end
        req_params.each { |key|
        ...
        if auth_req['qop'] == "auth" || auth_req['qop'] == "auth-int"
          param2 = %w[nonce nc cnonce qop].map { |key|
            auth_req[key]
          }.join(':')
        ...
        if digest != auth_req['response']
          ...
          if @use_opaque
            ...
            opaque_struct.nc    = format("%08x", (auth_req['nc'].hex + 1))
          end
          if auth_req['qop'] == "auth" || auth_req['qop'] == "auth-int"
            %w[qop cnonce nc].each { |key|
      ...
      def split_param_value(string)
        ...
        string.scan(/\G\s*([\w\-.*%!]+)=\s*(?:"((?>\\.|[^"])*)"|([^,"]*))\s*,?/) do
          ret[::Regexp.last_match(1)] = ::Regexp.last_match(3) || ::Regexp.last_match(2).gsub(/\\(.)/, "\\1")
        end
        ret
      end

      def generate_next_nonce(req)
        now = format("%012d", req.request_time.to_i)
        pk  = hexdigest(now, @instance_key)[0, 32]
        [now + ":" + pk].pack("m0") # it has 60 length of chars.
      end
      ...
      def check_nonce(req, auth_req)
        ...
        pub_time, pk = nonce.unpack1("m*").split(":", 2)
        if !pub_time || !pk
    ...
    class ProxyDigestAuth < DigestAuth
      ...
      def check_uri(_req, _auth_req) # :nodoc:
        true
      end
    end
  end
