...
module Rabrick
  ...
  #
  #   proxy =
  #     Rabrick::HTTPProxyServer.new Port: 8000, ProxyContentHandler: handler
  ...
  class HTTPProxyServer < HTTPServer
    ...
    def do_CONNECT(req, res)
      ...
      begin
        Rabrick::RactorLogger.debug("CONNECT: upstream proxy is `#{host}:#{port}'.")
        os = TCPSocket.new(host, port) # origin server

        if proxy
          Rabrick::RactorLogger.debug("CONNECT: sending a Request-Line")
          os << proxy_request_line << CRLF
          Rabrick::RactorLogger.debug("CONNECT: > #{proxy_request_line}")
          if credentials
            Rabrick::RactorLogger.debug("CONNECT: sending credentials")
            os << "Proxy-Authorization: " << credentials << CRLF
          end
          os << CRLF
          proxy_status_line = os.gets(LF)
          Rabrick::RactorLogger.debug("CONNECT: read Status-Line from the upstream server")
          Rabrick::RactorLogger.debug("CONNECT: < #{proxy_status_line}")
        ...
        Rabrick::RactorLogger.debug("CONNECT #{host}:#{port}: succeeded")
        res.status = HTTPStatus::RC_OK
      rescue StandardError => e
        Rabrick::RactorLogger.debug("CONNECT #{host}:#{port}: failed `#{e.message}'")
        res.set_error(e)
      ...
      begin
        while fds = IO.select([ua, os])
          if fds[0].member?(ua)
            ...
            Rabrick::RactorLogger.debug("CONNECT: #{buf.bytesize} byte from User-Agent")
            os.write(buf)
          elsif fds[0].member?(os)
            buf = os.readpartial(1024)
            Rabrick::RactorLogger.debug("CONNECT: #{buf.bytesize} byte from #{host}:#{port}")
            ua.write(buf)
          end
        end
      rescue StandardError
        os.close
        Rabrick::RactorLogger.debug("CONNECT #{host}:#{port}: closed")
      end
    ...
    def choose_header(src, dst)
      ...
      src.each { |key, value|
        ...
        if HopByHop.member?(key)          || # RFC2616: 13.5.1
          ...
          Rabrick::RactorLogger.debug("choose_header: `#{key}: #{value}'")
          next
        end
        dst[key] = value
      }
      ...
      req_fib = Fiber.new do
        http.start do
          ...
          http.request(http_req) do |response|
            ...
            # stream Net::HTTP::HTTPResponse to Rabrick::HTTPResponse
